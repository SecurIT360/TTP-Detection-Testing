...
# Version 1.1
# Author: Oddvar Moe
# Modified: Spencer Alessi July 2022
...
# create the folder where we will save our "malicious" CMSTP.inf file
$InfDirectory = split-path $InfFileLocation -Parent
if (Test-Path $InfDirectory) {
    Remove-Item -Recurse $InfDirectory -Force
    New-Item -ItemType Directory $InfDirectory | Out-Null
} else {
    New-Item -ItemType Directory $InfDirectory | Out-Null
}
